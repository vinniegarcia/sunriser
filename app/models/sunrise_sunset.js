...
var Sunriser = (function () {
    ...
    SunriseSunset.prototype = {
        ...
        makeTime: function (decimal) {
            if (decimal)
            {
                var timeValues = {
                    hour: 0, min: 0, sec: 0, meridian: 'AM'
                };
                timeValues.hour = parseInt(decimal);
                if (timeValues.hour < 1)
                {
                    timeValues.hour = 12;
                    timeValues.meridian = "AM";
                }
                else if (timeValues.hour > 12)
                {
                    timeValues.hour -= 12;
                    timeValues.meridian = "PM";
                }
                else if (timeValues.hour == 12)
                {
                    timeValues.meridian = "PM";
                ...
                decimal *= 60;
                timeValues.min = parseInt(decimal);
                if (timeValues.min < 10)
                {
                    timeValues.min = "0" + timeValues.min;
                }
                decimal -= parseInt(decimal);
                decimal *= 60;
                timeValues.sec = parseInt(decimal);
                if (timeValues.sec < 10)
                {
                    timeValues.sec = "0" + timeValues.sec;
                }
                return timeValues.hour  + ":" + timeValues.min + ":" + timeValues.sec + " " + timeValues.meridian;
            }
            else
            {
                return "00:00:00"
        ...
        sunriseUtcFriendly: function () {
            return this.makeTime(this.sunriseUtcHours());
        },
    
        sunsetUtcFriendly: function () {
            return this.makeTime(this.sunsetUtcHours());
        },
    
        sunriseLocalFriendly: function (offset) {
            return this.makeTime(this.sunriseLocalHours(offset));
        },
    
        sunsetLocalFriendly: function (offset) {
            return this.makeTime(this.sunsetLocalHours(offset));
        },
